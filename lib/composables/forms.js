...
function form(form, cfgExtend=null, inputProps=null){
    ...
    const data = ref(getData(form.value.data, form.value.fields, _fieldType));
    const errors = ref(getErrors(form.value.errors, form.value.fields));
    return{
        data: data,
        errors: errors,
        ...
        return {
            ...
            fieldCfg(fname){
                ...
                ret["onUpdate:modelValue"] = function(val){
                    emit("update:modelValue", fname, idx, val);
                }
                return ret;
            }
        };
...
export function formsetValues(form){
    ...
    return {
        ...
        data,
        events: {
            "onUpdate:modelValue": (fname, idx, value)=>{
                if(fname === null){
                    data.value[idx] = value;
                }
                else{
                    data.value[idx][fname] = value;
                }
            }
        }
    }
